(ns tsumugu.cacao
  "Agent-side CACAO issuance (JVM). The tsumugu actor mints its OWN
  server-verifiable CACAO to authenticate to a kotoba-server pod (kotobase.net)
  — no human-handed token. Ported from `itonami.cacao` (keep in sync); the
  SIWE/wire builders below are a faithful copy of the proven byte-exact pure
